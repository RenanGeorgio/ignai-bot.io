...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const userPrefersSystemMode = !user?.preferredAppAppearance || user.preferredAppAppearance === 'system'
    const computedColorMode = userPrefersSystemMode ? systemColorScheme : user?.preferredAppAppearance

    if (computedColorMode === currentColorScheme) {
      return
    }
  ...
  useEffect(() => {
    if (isDefined(user) || isNotDefined(session)) {
      return
    }

    setCurrentWorkspaceId(localStorage.getItem('currentWorkspaceId') ?? undefined);
  ...
  const saveUser = useDebouncedCallback(async (updates: Partial<User>) => {
    if (isNotDefined(user)) {
      return
    }
