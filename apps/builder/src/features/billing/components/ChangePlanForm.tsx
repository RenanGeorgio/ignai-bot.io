...
export const ChangePlanForm = ({ workspace, currentRole, excludedPlans }: Props) => {
  ...
  if (data?.subscription?.cancelDate || data?.subscription?.status === 'past_due') {
    return null
  }

  const isSubscribed = (workspace.plan === Plan.STARTER || workspace.plan === Plan.PRO) && workspace.stripeId

  if (workspace.plan !== Plan.FREE && !isSubscribed) {
    return null
  }
  ...
  if (currentRole !== WorkspaceRole.ADMIN) {
    return (
      <Text>
        ...
        administrador do workspace para alterar o plano.
      </Text>
    );
  }
  ...
  return (
    <Stack spacing={6}>
      ...
      <Text color="gray.500">
        ...
        <TextLink href={'https://ignaibot.com/enterprise-lead-form'} isExternal>
